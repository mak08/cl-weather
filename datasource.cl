...
(defclass noaa-datasource (datasource range-download)
  ())
...
(defclass noaa-rtofs-current (noaa-datasource datakind-current file-download)
  ((name :initform "rtofs")
...
(defclass ecmwf-datasource (datasource range-download)
  ())
...
(defgeneric load-forecast (datasource step)
  (:documentation
   "Return requested U/V data from Cache, fill cache if needed."))

(defgeneric load-datasource-forecasts (datasource step)
  (:documentation
